...
export default function Header(){
    ...
    return (
        <div className="lg:fixed lg:w-full z-[100] lg:bg-gray-900/30 lg:backdrop-blur-sm">
            <div className="flex w-full py-[1rem] lg:px-8 px-[20px] border-b border-gray-800 z-100">
                <Image 
                    src={brainwave}
                    width={180}
                    height={40}
                ...
                <div className="font-code-pro text-gray-300 hidden lg:flex ml-auto uppercase text-xs font-bold ">
                    {navigationButton.map((item, key) => (
                        key == 1 ? 
                        <Button key={key} href={item.url} className="h-full px-4 text-white" border>
                            {item.title}
                        </Button>
                        :
